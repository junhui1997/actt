...
class ACTPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        self.kl_weight = args_override['kl_weight']  # 是个数值权重
        self.vq = args_override['vq']  # bool
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None, vq_sample=None):
        ...
        if actions is not None: # training time
            actions = actions[:, :self.model.num_queries]  # 这两步感觉没有什么必要，因为在dataloader里面就已经给缩放成chunk_size了
            is_pad = is_pad[:, :self.model.num_queries]  # [bs,chunk_size)
            ...
            all_l1 = F.l1_loss(actions, a_hat, reduction='none')  # [bs,chunk_size,act_dim]
            l1 = (all_l1 * ~is_pad.unsqueeze(-1)).mean()  # *和～都是按位操作哦，～是取反，从这里使用了
            loss_dict['l1'] = l1
