...
def main(args):
    ...
    if policy_class == 'ACT':
        ...
        policy_config = {'lr': args['lr'],
                         ...
                         'vq_dim': args['vq_dim'],
                         'action_dim': action_dim,  # 这里原本是写死的16
                         'state_dim': state_dim,  # add by me
...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    collect_video = 1
    if collect_video:
        output_file_name = 'rollout_' + ckpt_name.split('.')[0] + '.mp4'  # 输出视频文件名
        output_file = os.path.join(ckpt_dir, output_file_name)
        ...
        if step % validate_every == 0:
            ...
            with torch.inference_mode():
                ...
                for batch_idx, data in enumerate(val_dataloader):
                    # 这里返回的是一个dic，act model返回的是{kl,l1,loss},对应的是每个batch的
