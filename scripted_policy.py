...
class PickAndTransferPolicy(BasePolicy):
    ...
    def generate_trajectory(self, ts_first):
        ...
        # 右侧的转变为抓取姿态之后就没再变过，抓取时候的box_xyz返回的是质心的么？所以中间移动的这个尺寸和xyz是不是相关，之后朝x正方向再移动一些（屏幕右侧）
        self.right_trajectory = [
            {"t": 0, "xyz": init_mocap_pose_right[:3], "quat": init_mocap_pose_right[3:], "gripper": 0},  # sleep # 初始位置啥也不干
...
def test_policy(task_name):
    ...
    if 'sim_transfer_cube' in task_name:
        ...
        raise NotImplementedError
    action_spec = env.action_spec()  # action space这里只有四个对应的是actuator
    for episode_idx in range(2):
        ts = env.reset()
        episode = [ts]
        if onscreen_render:
            ax = plt.subplot()
            plt_img = ax.imshow(ts.observation['images']['angle'])  # 这里原本他写的是angle
            plt.ion()
        ...
        for step in range(episode_len):
            ...
            episode.append(ts)
            if onscreen_render:
                plt_img.set_data(ts.observation['images']['angle'])  # 这里原本他写的是angle
                plt.pause(0.02)
        plt.close()
