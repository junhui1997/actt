...
def reparametrize(mu, logvar):
    std = logvar.div(2).exp()  # 逐向量e^(x/2)
    eps = Variable(std.data.new(std.size()).normal_())  # 从正态分布中随机采样，这里好像是老版本的写法
    return mu + std * eps


# 返回了一个sinusoid pos embedding
...
class DETRVAE(nn.Module):
    ...
    def encode(self, qpos, actions=None, is_pad=None, vq_sample=None):
        ...
        if self.encoder is None:
            ...
            # cvae encoder
            is_training = actions is not None # train or val # 这里是个判别句返回bool
            ### Obtain latent z from action sequence
            if is_training:
                # project action sequence to embedding dim, and concat with a CLS token
                action_embed = self.encoder_action_proj(actions) # (bs, seq or chunk_size, hidden_dim)
                ...
                cls_joint_is_pad = torch.full((bs, 2), False).to(qpos.device) # False: not a padding # (bs,2)
                is_pad = torch.cat([cls_joint_is_pad, is_pad], axis=1)  # (bs, seq+2)
                # obtain position embedding
                pos_embed = self.pos_table.clone().detach()   #(1,2+seq，dim）写在register buffer中的不会被直接声明
                ...
                if self.vq:
                    ...
                    latent_input = self.latent_out_proj(latent_sample)  # (bs,dim)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, vq_sample=None):
        ...
        """
        # encoder这里没有使用image信息 # 但是用了一个transformer的encoder部分
        ...
        if self.backbones is not None:
            ...
            # fold camera dimension into width dimension # cat不会改变维数  # pos是添加pos embedding之后的 #沿着w方向来拼接图片
            src = torch.cat(all_cam_features, axis=3)  # [bs,image_c,w*image_num,h]
            pos = torch.cat(all_cam_pos, axis=3)       # 同上
...
class CNNMLP(nn.Module):
    def __init__(self, backbones, state_dim, camera_names, action_dim):
        ...
        self.action_dim = action_dim  # TODO add by me, 6*2+2+2=16(机械臂+两个夹爪（每个夹爪一个自由度）+底盘角速度&线速度）
        if backbones is not None:
            self.backbones = nn.ModuleList(backbones)
            backbone_down_projs = []
            for backbone in backbones:
                # 进一步对channel维度进行降低，512 or 1024 ->32
    ...
    def forward(self, qpos, image, env_state, actions=None):
        ...
        for cam_id, cam_name in enumerate(self.camera_names):
            features, pos = self.backbones[cam_id](image[:, cam_id])
            features = features[0]  # take the last layer feature #[batch_size,num_channel512,height,width]
            pos = pos[0] # not used
            all_cam_features.append(self.backbone_down_projs[cam_id](features))
        # flatten everything # [batch_size,-1]
        flattened_features = []
...
def build_cnnmlp(args):
    state_dim = args.state_dim  # TODO hardcode # 这里的dim自己编的
    # 这里的backbone是为了提取视觉特征
    # From state
    # backbone = None # from state for now, no need for conv nets
    # From image
    backbones = []
    ...
    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)  # 统计para的方式
    print("number of parameters: %.2fM" % (n_parameters/1e6,))
