...
class SimpleRetention(nn.Module):
    ...
    def forward(self, X):
        ...
        Q = self.xpos(Q)
        K = self.xpos(K, downscale=True)  # [b,s,d]
        # 在这之前都没有改变维数
        # [b,s,d or 2*d]
        V = X @ self.W_V
        # [b,s,d]@[b,d,s]*[1,s,s]->[b,s,s]
...
class MultiScaleRetention(nn.Module):
    def __init__(self, hidden_size, heads, double_v_dim=False):
        ...
        self.gammas = (1 - torch.exp(torch.linspace(math.log(1/32), math.log(1/512), heads))).detach().cpu().tolist()  # gamma是一个list
