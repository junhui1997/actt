...
def make_sim_env(task_name):
    ...
    if 'sim_transfer_cube' in task_name:
        ...
        xml_path = os.path.join(XML_DIR, f'new_panda/single_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = SingleCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)
    else:
...
class SingleTask(base.Task):
    ...
    def before_step(self, action, physics):
        ...
        # left_arm_action = [-0.04375997  ,0.57270117, -0.03561139 ,-1.60982029 , 2.41749995 , 3.02822837 ,-2.44066306]# action[:7]
        # left_arm_action[0]+=0.005*self.count
        # left_arm_action[1] += 0.005*self.count
        # left_arm_action[2] += 0.005 * self.count
        #left_arm_action[3] -= 0.005 * self.count
        #print(left_arm_action[3])
        # left_arm_action[4] += 0.005 * self.count
        # left_arm_action[5] += 0.005 * self.count
        # left_arm_action[6] += 0.005 * self.count

        # self.count+=1
        normalized_left_gripper_action = action[7]

        left_gripper_action = PANDA_GRIPPER_POSITION_UNNORMALIZE_FN(normalized_left_gripper_action)
    ...
    def get_qpos(physics):
        ...
        left_qpos_raw = qpos_raw[:9]  # 7个关节+两个开关量
    ...
    def get_observation(self, physics):
        ...
        # obs['images']['left_wrist'] = physics.render(height=480, width=640, camera_id='left_wrist') #by me
        # obs['images']['right_wrist'] = physics.render(height=480, width=640, camera_id='right_wrist') # by me
        obs['images']['angle'] = physics.render(height=480, width=640, camera_id='angle')   # add by me
...
class SingleCubeTask(SingleTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 2
    ...
    def initialize_episode(self, physics):
        ...
        with physics.reset_context():
            physics.named.data.qpos[:9] = START_ARM_POSE_S
            START_ARM_POSE_S[7] = START_ARM_POSE_S[7]  # 看一下这里control时候需要的数值到底
            np.copyto(physics.data.ctrl, START_ARM_POSE_S[:8])
